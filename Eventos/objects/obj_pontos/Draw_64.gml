/// @description Insert description here
// You can write your code in this editor

//Desenhar os pontos na tela
draw_text(32, 60, "Pontos: " + string(global.pontos));

#region Atividade 6/35

/*
	Modulo 6/35
	
	Criar uma variavel global chamada de global.texto
	Vai iniciar sem valor
	Ela vai me dar o nome da atual instancia que pode se mover
	Exibir o nome usando o obj pontos
	Escreva o nome de cada instancia nela
*/

//Desenhando o nome da instancia que estou controlando na GUI
draw_text(32, 80, "Instancia atual: " + string(global.texto));

#endregion

draw_text(32, 100, "Tempo: " + string(global.tempo));

